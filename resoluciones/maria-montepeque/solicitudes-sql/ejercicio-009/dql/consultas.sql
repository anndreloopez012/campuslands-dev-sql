...
SELECT id_entrega, id_proyecto, tipo_evento, fecha_evento
FROM entregas
...
FROM proyectos p
JOIN entregas e ON e.id_proyecto = p.id_proyecto
GROUP BY p.id_proyecto, p.nombre_proyecto
ORDER BY total_eventos DESC;

-- 4. Historico ordenado cronologicamente: que paso y cuando paso.
SELECT id_entrega, id_proyecto, tipo_evento, fecha_evento, estado
FROM entregas
...
WHERE e.fecha_evento = (
    SELECT MAX(fecha_evento) FROM entregas WHERE id_proyecto = p.id_proyecto
)

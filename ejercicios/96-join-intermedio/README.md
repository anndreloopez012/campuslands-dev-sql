...
Trabajara sobre un sistema de registro de campers. El negocio necesita administrar campers inscritos en rutas de entrenamiento y practicar especificamente `JOIN` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-96/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-96
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/96-join-intermedio/plantilla resoluciones/apellido-nombre/ejercicio-96
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-96
git commit -m "feat(sql): resolver ejercicio 96"
git push -u origin alumno/apellido-nombre/ejercicio-96
```

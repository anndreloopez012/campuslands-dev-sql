...
Trabajara sobre un sistema de agenda de citas. El negocio necesita administrar citas programadas por fecha y practicar especificamente `INSERT` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-71/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-71
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/71-insert-basico/plantilla resoluciones/apellido-nombre/ejercicio-71
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-71
git commit -m "feat(sql): resolver ejercicio 71"
git push -u origin alumno/apellido-nombre/ejercicio-71
```

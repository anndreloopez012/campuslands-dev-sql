...
Trabajara sobre un sistema de ventas de cafeteria. El negocio necesita administrar ventas diarias de una cafeteria y practicar especificamente `DELETE` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-79/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-79
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/79-delete-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-79
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-79
git commit -m "feat(sql): resolver ejercicio 79"
git push -u origin alumno/apellido-nombre/ejercicio-79
```

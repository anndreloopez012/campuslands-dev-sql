...
La empresa tiene un archivo plano para administrar biblioteca con libros que tienen varios autores y prestamos repetidos. La informacion se captura en una sola tabla con columnas como:

```text
libro, autores, generos, usuario, prestamos, fechas_devolucion
```
...
Separar autores, libros, generos, usuarios, prestamos y tablas puente.
...
```text
resoluciones/apellido-nombre/ejercicio-37/
├── README.md
├── analisis/
│   └── normalizacion.md
├── diagramas/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-37
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/37-normalizacion-biblioteca-multiautor/plantilla resoluciones/apellido-nombre/ejercicio-37
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-37
git commit -m "feat(sql): resolver normalizacion ejercicio 37"
git push -u origin alumno/apellido-nombre/ejercicio-37
```

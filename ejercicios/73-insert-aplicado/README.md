...
Registrar datos individuales y multiples respetando restricciones.
...
Trabajara sobre un sistema de inventario de dispositivos. El negocio necesita administrar productos tecnologicos en bodega y practicar especificamente `INSERT` sin dejar de usar las bases normales de SQL.
...
5. Documente en `README.md` de su solucion que aprendio del tema `INSERT`.

## Requisitos especificos

- El ejercicio debe demostrar claramente el uso de `INSERT`.
...
5. Una consulta que demuestre que `INSERT` cumple su proposito.

## Entregables

```text
resoluciones/apellido-nombre/ejercicio-73/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-73
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/73-insert-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-73
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-73
git commit -m "feat(sql): resolver ejercicio 73"
git push -u origin alumno/apellido-nombre/ejercicio-73
```

...
Trabajara sobre un sistema de torneo de videojuegos. El negocio necesita administrar partidas y puntajes por equipo y practicar especificamente `GROUP BY` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-90/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-90
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/90-group-by-intermedio/plantilla resoluciones/apellido-nombre/ejercicio-90
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-90
git commit -m "feat(sql): resolver ejercicio 90"
git push -u origin alumno/apellido-nombre/ejercicio-90
```

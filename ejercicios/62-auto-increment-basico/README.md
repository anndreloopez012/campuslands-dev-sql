...
Trabajara sobre un sistema de registro de campers. El negocio necesita administrar campers inscritos en rutas de entrenamiento y practicar especificamente `AUTO_INCREMENT` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-62/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-62
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/62-auto-increment-basico/plantilla resoluciones/apellido-nombre/ejercicio-62
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-62
git commit -m "feat(sql): resolver ejercicio 62"
git push -u origin alumno/apellido-nombre/ejercicio-62
```

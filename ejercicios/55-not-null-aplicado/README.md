...
Distinguir campos obligatorios de campos opcionales y validar datos minimos.
...
Trabajara sobre un sistema de inventario de dispositivos. El negocio necesita administrar productos tecnologicos en bodega y practicar especificamente `NOT NULL` sin dejar de usar las bases normales de SQL.
...
5. Documente en `README.md` de su solucion que aprendio del tema `NOT NULL`.

## Requisitos especificos

- El ejercicio debe demostrar claramente el uso de `NOT NULL`.
...
5. Una consulta que demuestre que `NOT NULL` cumple su proposito.

## Entregables

```text
resoluciones/apellido-nombre/ejercicio-55/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-55
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/55-not-null-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-55
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-55
git commit -m "feat(sql): resolver ejercicio 55"
git push -u origin alumno/apellido-nombre/ejercicio-55
```

...
La empresa tiene un archivo plano para administrar inventario con proveedores y movimientos repetidos. La informacion se captura en una sola tabla con columnas como:

```text
producto, categoria, proveedor, telefono_proveedor, entradas, salidas, bodega
```
...
Separar categorias, productos, proveedores, bodegas y movimientos.
...
```text
resoluciones/apellido-nombre/ejercicio-35/
├── README.md
├── analisis/
│   └── normalizacion.md
├── diagramas/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-35
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/35-normalizacion-inventario-proveedores/plantilla resoluciones/apellido-nombre/ejercicio-35
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-35
git commit -m "feat(sql): resolver normalizacion ejercicio 35"
git push -u origin alumno/apellido-nombre/ejercicio-35
```

...
Disenar e implementar una base de datos relacional en SQLite para un contexto de plataforma academica integral. El ejercicio se enfoca en proyecto integral con modelo relacional y consultas avanzadas.

## Contexto

Una organizacion necesita dejar de manejar informacion en hojas sueltas y pasar a una base de datos relacional. Actualmente registra manualmente campers, rutas, trainers, modulos, evaluaciones y asistencias, lo que provoca duplicidad, errores de captura y dificultad para generar reportes.
...
La solucion debe tener entre 5 y 6 tablas justificadas. Cada tabla debe tener una responsabilidad clara.
...
```text
resoluciones/apellido-nombre/ejercicio-30/
├── README.md
├── diagramas/
│   └── diagrama-er.png
├── ddl/
...
5. Copie la carpeta `plantilla/` dentro de `resoluciones/apellido-nombre/ejercicio-30/`.
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-30
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/30-proyecto-integrador-sql/plantilla resoluciones/apellido-nombre/ejercicio-30
```
...
- Titulo claro, por ejemplo: `feat(sql): resolver ejercicio 30`

...
Trabajara sobre un sistema de biblioteca tecnica. El negocio necesita administrar prestamos de libros tecnicos y practicar especificamente `UNIQUE` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-50/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-50
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/50-unique-basico/plantilla resoluciones/apellido-nombre/ejercicio-50
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-50
git commit -m "feat(sql): resolver ejercicio 50"
git push -u origin alumno/apellido-nombre/ejercicio-50
```

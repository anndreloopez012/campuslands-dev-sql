...
Eliminar datos de forma controlada y justificar cuando conviene baja logica.
...
Trabajara sobre un sistema de registro de campers. El negocio necesita administrar campers inscritos en rutas de entrenamiento y practicar especificamente `DELETE` sin dejar de usar las bases normales de SQL.
...
5. Documente en `README.md` de su solucion que aprendio del tema `DELETE`.

## Requisitos especificos

- El ejercicio debe demostrar claramente el uso de `DELETE`.
...
5. Una consulta que demuestre que `DELETE` cumple su proposito.

## Entregables

```text
resoluciones/apellido-nombre/ejercicio-78/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-78
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/78-delete-intermedio/plantilla resoluciones/apellido-nombre/ejercicio-78
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-78
git commit -m "feat(sql): resolver ejercicio 78"
git push -u origin alumno/apellido-nombre/ejercicio-78
```

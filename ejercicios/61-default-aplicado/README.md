...
Trabajara sobre un sistema de ventas de cafeteria. El negocio necesita administrar ventas diarias de una cafeteria y practicar especificamente `DEFAULT` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-61/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-61
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/61-default-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-61
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-61
git commit -m "feat(sql): resolver ejercicio 61"
git push -u origin alumno/apellido-nombre/ejercicio-61
```

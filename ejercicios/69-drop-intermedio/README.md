...
Trabajara sobre un sistema de agenda de citas. El negocio necesita administrar citas programadas por fecha y practicar especificamente `DROP` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-69/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-69
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/69-drop-intermedio/plantilla resoluciones/apellido-nombre/ejercicio-69
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-69
git commit -m "feat(sql): resolver ejercicio 69"
git push -u origin alumno/apellido-nombre/ejercicio-69
```

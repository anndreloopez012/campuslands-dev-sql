...
La empresa tiene un archivo plano para administrar reservas de hotel con huespedes y servicios en columnas repetidas. La informacion se captura en una sola tabla con columnas como:

```text
huesped, documento, habitacion, servicios, costos_servicios, fechas
```
...
Separar huespedes, habitaciones, reservas, servicios y detalle_servicio.
...
```text
resoluciones/apellido-nombre/ejercicio-36/
├── README.md
├── analisis/
│   └── normalizacion.md
├── diagramas/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-36
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/36-normalizacion-hotel-reservas/plantilla resoluciones/apellido-nombre/ejercicio-36
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-36
git commit -m "feat(sql): resolver normalizacion ejercicio 36"
git push -u origin alumno/apellido-nombre/ejercicio-36
```

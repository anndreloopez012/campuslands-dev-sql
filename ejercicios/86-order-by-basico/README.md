...
Trabajara sobre un sistema de biblioteca tecnica. El negocio necesita administrar prestamos de libros tecnicos y practicar especificamente `ORDER BY` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-86/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-86
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/86-order-by-basico/plantilla resoluciones/apellido-nombre/ejercicio-86
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-86
git commit -m "feat(sql): resolver ejercicio 86"
git push -u origin alumno/apellido-nombre/ejercicio-86
```

...
La empresa tiene un archivo plano para administrar pedidos de restaurante con platos multiples en una sola fila. La informacion se captura en una sola tabla con columnas como:

```text
mesa, mesero, platos, categorias, precios, cantidades, fecha_pedido
```
...
Separar mesas, meseros, platos, pedidos y detalle_pedido.
...
```text
resoluciones/apellido-nombre/ejercicio-32/
├── README.md
├── analisis/
│   └── normalizacion.md
├── diagramas/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-32
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/32-normalizacion-pedidos-restaurante/plantilla resoluciones/apellido-nombre/ejercicio-32
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-32
git commit -m "feat(sql): resolver normalizacion ejercicio 32"
git push -u origin alumno/apellido-nombre/ejercicio-32
```

...
Modificar datos existentes con condiciones seguras usando WHERE.
...
Trabajara sobre un sistema de registro de campers. El negocio necesita administrar campers inscritos en rutas de entrenamiento y practicar especificamente `UPDATE` sin dejar de usar las bases normales de SQL.
...
5. Documente en `README.md` de su solucion que aprendio del tema `UPDATE`.

## Requisitos especificos

- El ejercicio debe demostrar claramente el uso de `UPDATE`.
...
5. Una consulta que demuestre que `UPDATE` cumple su proposito.

## Entregables

```text
resoluciones/apellido-nombre/ejercicio-76/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-76
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/76-update-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-76
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-76
git commit -m "feat(sql): resolver ejercicio 76"
git push -u origin alumno/apellido-nombre/ejercicio-76
```

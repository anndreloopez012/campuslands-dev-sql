...
Trabajara sobre un sistema de ventas de cafeteria. El negocio necesita administrar ventas diarias de una cafeteria y practicar especificamente `Tipos de datos` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-43/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-43
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/43-tipos-datos-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-43
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-43
git commit -m "feat(sql): resolver ejercicio 43"
git push -u origin alumno/apellido-nombre/ejercicio-43
```

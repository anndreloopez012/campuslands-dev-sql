...
Trabajara sobre un sistema de inventario de dispositivos. El negocio necesita administrar productos tecnologicos en bodega y practicar especificamente `DELETE` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-77/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-77
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/77-delete-basico/plantilla resoluciones/apellido-nombre/ejercicio-77
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-77
git commit -m "feat(sql): resolver ejercicio 77"
git push -u origin alumno/apellido-nombre/ejercicio-77
```

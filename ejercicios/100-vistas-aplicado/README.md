...
Trabajara sobre un sistema de biblioteca tecnica. El negocio necesita administrar prestamos de libros tecnicos y practicar especificamente `Vistas` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-100/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-100
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/100-vistas-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-100
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-100
git commit -m "feat(sql): resolver ejercicio 100"
git push -u origin alumno/apellido-nombre/ejercicio-100
```

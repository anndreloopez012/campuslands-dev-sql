...
Crear indices para busquedas frecuentes y analizar cuando convienen.
...
Trabajara sobre un sistema de inventario de dispositivos. El negocio necesita administrar productos tecnologicos en bodega y practicar especificamente `Indices` sin dejar de usar las bases normales de SQL.
...
5. Documente en `README.md` de su solucion que aprendio del tema `Indices`.

## Requisitos especificos

- El ejercicio debe demostrar claramente el uso de `Indices`.
...
5. Una consulta que demuestre que `Indices` cumple su proposito.

## Entregables

```text
resoluciones/apellido-nombre/ejercicio-109/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-109
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/109-indices-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-109
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-109
git commit -m "feat(sql): resolver ejercicio 109"
git push -u origin alumno/apellido-nombre/ejercicio-109
```

...
Trabajara sobre un sistema de inventario de dispositivos. El negocio necesita administrar productos tecnologicos en bodega y practicar especificamente `HAVING` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-93/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-93
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/93-having-intermedio/plantilla resoluciones/apellido-nombre/ejercicio-93
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-93
git commit -m "feat(sql): resolver ejercicio 93"
git push -u origin alumno/apellido-nombre/ejercicio-93
```

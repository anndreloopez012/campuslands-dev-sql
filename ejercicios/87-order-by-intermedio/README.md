...
Trabajara sobre un sistema de agenda de citas. El negocio necesita administrar citas programadas por fecha y practicar especificamente `ORDER BY` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-87/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-87
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/87-order-by-intermedio/plantilla resoluciones/apellido-nombre/ejercicio-87
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-87
git commit -m "feat(sql): resolver ejercicio 87"
git push -u origin alumno/apellido-nombre/ejercicio-87
```

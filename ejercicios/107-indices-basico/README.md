...
Trabajara sobre un sistema de agenda de citas. El negocio necesita administrar citas programadas por fecha y practicar especificamente `Indices` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-107/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-107
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/107-indices-basico/plantilla resoluciones/apellido-nombre/ejercicio-107
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-107
git commit -m "feat(sql): resolver ejercicio 107"
git push -u origin alumno/apellido-nombre/ejercicio-107
```

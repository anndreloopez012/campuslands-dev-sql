...
Modificar estructuras existentes agregando columnas, renombrando tablas o ajustando campos.
...
Trabajara sobre un sistema de ventas de cafeteria. El negocio necesita administrar ventas diarias de una cafeteria y practicar especificamente `ALTER TABLE` sin dejar de usar las bases normales de SQL.
...
5. Documente en `README.md` de su solucion que aprendio del tema `ALTER TABLE`.

## Requisitos especificos

- El ejercicio debe demostrar claramente el uso de `ALTER TABLE`.
...
5. Una consulta que demuestre que `ALTER TABLE` cumple su proposito.

## Entregables

```text
resoluciones/apellido-nombre/ejercicio-65/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-65
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/65-alter-table-basico/plantilla resoluciones/apellido-nombre/ejercicio-65
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-65
git commit -m "feat(sql): resolver ejercicio 65"
git push -u origin alumno/apellido-nombre/ejercicio-65
```
...
| Criterio | Peso |
| --- | --- |
| Uso correcto de `ALTER TABLE` | 35% |

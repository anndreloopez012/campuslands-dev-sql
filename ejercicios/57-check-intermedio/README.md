...
Trabajara sobre un sistema de inventario de dispositivos. El negocio necesita administrar productos tecnologicos en bodega y practicar especificamente `CHECK` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-57/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-57
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/57-check-intermedio/plantilla resoluciones/apellido-nombre/ejercicio-57
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-57
git commit -m "feat(sql): resolver ejercicio 57"
git push -u origin alumno/apellido-nombre/ejercicio-57
```

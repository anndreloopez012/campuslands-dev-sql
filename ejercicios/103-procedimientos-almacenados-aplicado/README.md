...
Trabajara sobre un sistema de agenda de citas. El negocio necesita administrar citas programadas por fecha y practicar especificamente `Procedimientos almacenados` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-103/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-103
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/103-procedimientos-almacenados-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-103
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-103
git commit -m "feat(sql): resolver ejercicio 103"
git push -u origin alumno/apellido-nombre/ejercicio-103
```

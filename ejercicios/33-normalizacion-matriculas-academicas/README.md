...
La empresa tiene un archivo plano para administrar matriculas con datos repetidos de estudiantes, rutas y trainers. La informacion se captura en una sola tabla con columnas como:

```text
estudiante, documento, ruta, trainer, modulos, horarios, notas
```
...
Separar estudiantes, rutas, trainers, modulos, matriculas y notas.
...
```text
resoluciones/apellido-nombre/ejercicio-33/
├── README.md
├── analisis/
│   └── normalizacion.md
├── diagramas/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-33
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/33-normalizacion-matriculas-academicas/plantilla resoluciones/apellido-nombre/ejercicio-33
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-33
git commit -m "feat(sql): resolver normalizacion ejercicio 33"
git push -u origin alumno/apellido-nombre/ejercicio-33
```

...
La empresa tiene un archivo plano para administrar ordenes de ecommerce con pagos, cupones y envios mezclados. La informacion se captura en una sola tabla con columnas como:

```text
comprador, productos, cupon, metodo_pago, direccion_envio, transportadora
```
...
Separar compradores, productos, ordenes, pagos, cupones y envios.
...
```text
resoluciones/apellido-nombre/ejercicio-38/
├── README.md
├── analisis/
│   └── normalizacion.md
├── diagramas/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-38
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/38-normalizacion-ecommerce-envios/plantilla resoluciones/apellido-nombre/ejercicio-38
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-38
git commit -m "feat(sql): resolver normalizacion ejercicio 38"
git push -u origin alumno/apellido-nombre/ejercicio-38
```

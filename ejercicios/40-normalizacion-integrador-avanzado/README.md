...
La empresa tiene un archivo plano para administrar caso integrado con ventas, clientes, productos, pagos y entregas. La informacion se captura en una sola tabla con columnas como:

```text
cliente, vendedor, productos, pagos, cuotas, entregas, estados
```
...
Normalizar hasta 3FN y justificar si alguna relacion requiere descomposicion adicional.
...
```text
resoluciones/apellido-nombre/ejercicio-40/
├── README.md
├── analisis/
│   └── normalizacion.md
├── diagramas/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-40
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/40-normalizacion-integrador-avanzado/plantilla resoluciones/apellido-nombre/ejercicio-40
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-40
git commit -m "feat(sql): resolver normalizacion ejercicio 40"
git push -u origin alumno/apellido-nombre/ejercicio-40
```

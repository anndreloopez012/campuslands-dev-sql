...
La empresa tiene un archivo plano para administrar nomina con empleados, cargos, departamentos y beneficios repetidos. La informacion se captura en una sola tabla con columnas como:
...
Separar empleados, cargos, departamentos, contratos, beneficios y asignaciones.
...
```text
resoluciones/apellido-nombre/ejercicio-39/
├── README.md
├── analisis/
│   └── normalizacion.md
├── diagramas/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-39
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/39-normalizacion-recursos-humanos/plantilla resoluciones/apellido-nombre/ejercicio-39
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-39
git commit -m "feat(sql): resolver normalizacion ejercicio 39"
git push -u origin alumno/apellido-nombre/ejercicio-39
```

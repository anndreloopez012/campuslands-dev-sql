...
Trabajara sobre un sistema de registro de campers. El negocio necesita administrar campers inscritos en rutas de entrenamiento y practicar especificamente `SELECT` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-80/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-80
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/80-select-basico/plantilla resoluciones/apellido-nombre/ejercicio-80
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-80
git commit -m "feat(sql): resolver ejercicio 80"
git push -u origin alumno/apellido-nombre/ejercicio-80
```

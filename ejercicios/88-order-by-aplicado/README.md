...
Ordenar resultados ascendente y descendente por una o varias columnas.
...
Trabajara sobre un sistema de torneo de videojuegos. El negocio necesita administrar partidas y puntajes por equipo y practicar especificamente `ORDER BY` sin dejar de usar las bases normales de SQL.
...
5. Documente en `README.md` de su solucion que aprendio del tema `ORDER BY`.

## Requisitos especificos

- El ejercicio debe demostrar claramente el uso de `ORDER BY`.
...
5. Una consulta que demuestre que `ORDER BY` cumple su proposito.

## Entregables

```text
resoluciones/apellido-nombre/ejercicio-88/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-88
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/88-order-by-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-88
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-88
git commit -m "feat(sql): resolver ejercicio 88"
git push -u origin alumno/apellido-nombre/ejercicio-88
```

...
Trabajara sobre un sistema de ventas de cafeteria. El negocio necesita administrar ventas diarias de una cafeteria y practicar especificamente `JOIN` sin dejar de usar las bases normales de SQL.
...
```text
resoluciones/apellido-nombre/ejercicio-97/
├── README.md
├── ddl/
│   └── schema.sql
├── dml/
...
git switch dev
git pull origin dev
git switch -c alumno/apellido-nombre/ejercicio-97
mkdir -p resoluciones/apellido-nombre
cp -R ejercicios/97-join-aplicado/plantilla resoluciones/apellido-nombre/ejercicio-97
```

Despues de resolver:

```bash
git add resoluciones/apellido-nombre/ejercicio-97
git commit -m "feat(sql): resolver ejercicio 97"
git push -u origin alumno/apellido-nombre/ejercicio-97
```
